...
class CopyService:
    ...
    async def generate(self, request: CopyGenerateRequest) -> CopyGenerateResponse:
        output, sources = self._graph.run(request)
...
4) 언어({language})와 채널({channel}) 특성을 반영할 것
        ...
        try:
            structured_model = self._model.with_structured_output(CopyStructuredOutput)
            ...
            if hasattr(result, "model_dump"):
                return CopyStructuredOutput.model_validate(result.model_dump())
            return self._mock_output(request, context)
        except Exception:
            return self._mock_output(request, context)
