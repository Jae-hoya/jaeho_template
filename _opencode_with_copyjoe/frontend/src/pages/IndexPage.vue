...
import { generateCopy, generateCopyLite, indexRagDocuments, uploadSourceFiles } from "../copyjoe/models-service/service"
...
async function onGenerate(payload: CopyGenerateRequest) {
  ...
  addTurn("user", `카피 생성 요청: ${payload.product_name} / objective=${payload.objective}`)
  ...
  try {
    ...
    addTurn("assistant", `생성 완료: head='${response.head}'`)
    addTurn("assistant", `버전 v${generationRound.value} 생성`)
  } catch (error) {
    errorMessage.value = toError(error)
    addTurn("assistant", `카피 생성 에러: ${errorMessage.value}`)
  } finally {
    loadingCopy.value = false
  }
}

async function onGenerateLite(payload: CopyLiteRequest) {
  if (loadingCopy.value || loadingLanding.value || loadingUpload.value) {
    return
  }
  ...
  errorMessage.value = ""
  loadingCopy.value = true
  addTurn("user", `간편 생성 요청: prompt='${payload.prompt.slice(0, 60)}...'`)
...
async function onUploadFiles(files: File[]) {
  ...
  addTurn("user", `RAG 파일 업로드 요청: ${files.map((item) => item.name).join(", ")}`)
  ...
  try {
    ...
    if (failedRows.length > 0) {
      const failedText = failedRows
        .slice(0, 5)
        .map((item) => `${item.file_name}(${item.error_code || "ERROR"})`)
        .join(", ")
      addTurn("assistant", `업로드 실패 파일: ${failedText}`)
    }

    if (successfulDocumentIds.length === 0) {
      addTurn("assistant", "인덱싱할 성공 문서가 없어 RAG 인덱싱은 건너뜁니다.")
      return
    }

    const indexed = await indexRagDocuments(successfulDocumentIds)
    addTurn("assistant", `RAG 인덱싱 완료: 문서 ${indexed.indexed_documents}개, 청크 ${indexed.indexed_chunks}개`)
...
<template>
  <main class="page">
    <section class="hero">
      <h1>Copyjoe Vue Landing</h1>
      <p>PRD 구조대로 `.vue` 파일 기반으로 구성한 랜딩/대화형 생성 화면입니다.</p>
      <p>
        LangChain/LangGraph는 백엔드에서 실행되고,
        이 화면은 API 결과를 대화형으로 확인하는 프론트입니다.
      </p>
    </section>
