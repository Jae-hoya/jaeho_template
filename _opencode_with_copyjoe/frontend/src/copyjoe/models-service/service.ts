...
export async function generateCopy(payload: CopyGenerateRequest): Promise<CopyGenerateResponse> {
  const response = await api.post<CopyGenerateResponse>("/api/v1/copy/generate", payload)
  return response.data
}

export async function generateCopyLite(payload: CopyLiteRequest): Promise<CopyLiteResponse> {
  const response = await api.post<CopyLiteResponse>("/api/v1/copy/generate-lite", payload)
  return response.data
}
