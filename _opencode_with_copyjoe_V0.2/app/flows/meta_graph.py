...
class MetaGuideState(TypedDict):
    response: CopyFormGuideResponse | None


class MetaCopyFormGuideGraph:
    def __init__(self) -> None:
        builder = StateGraph(MetaGuideState)
        builder.add_node("build", self._build_guide)
        builder.add_edge(START, "build")
        builder.add_edge("build", END)
        self._graph = builder.compile()

    def run(self) -> CopyFormGuideResponse:
        state = self._graph.invoke({"response": None})
        response = state.get("response")
        if response is None:
            raise RuntimeError("Meta guide graph failed to build response")
        return response

    def _build_guide(self, _: MetaGuideState) -> dict[str, object]:
        return {
            "response": CopyFormGuideResponse(
                fields=[
                    FieldGuideItem(
                        key="pain_point",
