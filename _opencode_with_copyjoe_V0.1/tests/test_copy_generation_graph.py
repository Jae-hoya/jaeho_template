...
def test_graph_collects_rag_and_web_context() -> None:
    ...
    graph = CopyGenerationGraph(
        rag_service=cast(Any, DummyRagService()),
        web_search_service=cast(Any, DummyWebSearchService()),
        generator=generator,
    )
...
def test_graph_preserves_extra_context_and_sources() -> None:
    captured: dict[str, str] = {}
    ...
    output, sources = graph.run(
        request,
        extra_context_blocks=["rendered landing context"],
        extra_sources=[
            SourceItem(
                source_type="web",
                title="landing",
    ...
    assert output.head == "h"
    assert "rendered landing context" in captured["context"]
    assert len(sources) == 1
    assert sources[0].url == "https://landing.example.com"
