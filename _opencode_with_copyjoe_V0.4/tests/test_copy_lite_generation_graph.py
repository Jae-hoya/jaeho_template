...
def test_copy_lite_graph_reuses_base_request_without_prompt_parsing() -> None:
    async def run_case() -> None:
        base_request = CopyGenerateRequest(
            product_name="Copyjoe",
            target_audience="마케터",
            pain_point="전환률이 낮다",
            differentiator="근거 기반 생성",
        ...
        def parse_prompt(_: str) -> tuple[CopyLiteParsedInput, list[str]]:
            raise AssertionError("parse_prompt should not be called when base_request is provided")
        ...
        def infer_language(prompt: str) -> str | None:
            return "en" if "영어" in prompt else None

        captured_context_blocks: list[str] = []
        ...
        async def generate_copy(
            ...
            captured_context_blocks.extend(list(extra_context_blocks or []))
            _ = extra_sources
            return CopyGenerateResponse(
                head="h",
                body="b",
                ...
                description="d",
                storyboard_outline=["1", "2", "3", "4"],
                rationale=f"lang={request.language}",
                sources=[],
            )
        ...
        assert response.normalized_request.product_name == "Copyjoe"
        assert response.normalized_request.target_audience == "마케터"
        assert response.normalized_request.objective == Objective.click
        assert response.normalized_request.language == "en"
        assert any("base_request 재사용" in item for item in response.assumptions)
        assert any("[사용자 피드백 기반 개선 요청]" in block for block in captured_context_blocks)
