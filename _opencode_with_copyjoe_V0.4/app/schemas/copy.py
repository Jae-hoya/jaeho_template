...
class CopyGenerateRequest(BaseModel):
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "use_rag": True,
                "top_k": 5,
                "rag_document_ids": ["document-id-1"],
            }
        }
    )
    ...
    rag_document_ids: list[str] | None = Field(
        default=None,
        description="RAG 검색을 특정 업로드 문서(document_id)로 제한할 때 사용",
    )
...
class CopyLiteRequest(BaseModel):
    ...
    base_request: CopyGenerateRequest | None = Field(
        default=None,
        description=(
            "이전 normalized_request를 그대로 재사용해 프롬프트 파싱 단계를 건너뛸 때 사용. "
            "refine 요청에서 지연을 줄이기 위한 옵션"
        ),
    )
