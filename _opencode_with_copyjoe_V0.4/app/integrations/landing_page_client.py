...
class LandingPageClient:
    def __init__(
        self,
        timeout_sec: int = 35,
        subprocess_timeout_buffer_sec: int = 4,
        network_idle_wait_ms: int = 700,
        fallback_timeout_sec: int = 8,
    ) -> None:
        self.timeout_sec = max(5, int(timeout_sec))
        self.subprocess_timeout_buffer_sec = max(1, int(subprocess_timeout_buffer_sec))
        self.network_idle_wait_ms = max(200, int(network_idle_wait_ms))
        self.fallback_timeout_sec = max(3, int(fallback_timeout_sec))
    ...
    def _analyze_with_playwright_subprocess(self, url: str) -> dict[str, Any]:
        ...
        wait_ms = self.network_idle_wait_ms
        script = r'''
import json
import sys
...
'''
        ...
        completed = subprocess.run(
            [sys.executable, "-c", script, url, str(timeout_ms), str(wait_ms)],
            capture_output=True,
            text=True,
            timeout=self.timeout_sec + self.subprocess_timeout_buffer_sec,
            check=False,
        )
    ...
    def _analyze_with_playwright_sync(self, url: str) -> dict[str, Any]:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                page.wait_for_load_state("networkidle", timeout=self.network_idle_wait_ms)
            except Exception:
                pass
    ...
    def _analyze_with_requests(self, url: str) -> dict[str, Any]:
        fallback_timeout = min(self.timeout_sec, self.fallback_timeout_sec)
