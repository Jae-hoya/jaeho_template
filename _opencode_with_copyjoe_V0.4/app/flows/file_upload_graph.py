...
class FileUploadState(TypedDict):
    ...
    response: FileUploadResponse | None


class FileUploadGraph:
    def __init__(self, settings: Settings, docling_client: DoclingClient, document_store: DocumentStore) -> None:
        self._settings = settings
        self._docling_client = docling_client
        self._document_store = document_store

        builder = StateGraph(FileUploadState)
        builder.add_node("validate", self._validate_batch)
        builder.add_node("process", self._process_files)
        builder.add_node("to_response", self._build_response)
        builder.add_edge(START, "validate")
        builder.add_edge("validate", "process")
        builder.add_edge("process", "to_response")
        builder.add_edge("to_response", END)
        self._graph = builder.compile()

    async def run(self, files: list[UploadFile]) -> FileUploadResponse:
        ...
        if response is None:
            return FileUploadResponse(total_files=len(files), success_count=0, failed_count=0, files=[])
        return response

    def _validate_batch(self, state: FileUploadState) -> dict[str, object]:
    ...
    async def _process_files(self, state: FileUploadState) -> dict[str, object]:
        files = list(state.get("files", []))
        items: list[UploadedFileItem] = []
        ...
        for file in files:
            ...
            try:
                text, conversion_meta = self._docling_client.convert_to_text_with_meta(source_path)
            ...
            items.append(
                UploadedFileItem(
                    ...
                    conversion_engine=str(conversion_meta.get("conversion_engine", "")) or None,
    ...
    def _build_response(self, state: FileUploadState) -> dict[str, object]:
        files = list(state.get("files", []))
