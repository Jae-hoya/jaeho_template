...
- 오케스트레이션: LangGraph 도메인 그래프 (`app/flows/*.py`)
- 랜딩 UI: Vue 3 SFC (`frontend/src/pages/IndexPage.vue`)
- 생성 API: `POST /api/v1/copy/generate` (prompt + style 중심 간편 모드 지원)
...
문서 변환 엔진은 `docling[easyocr,rapidocr]` 기준으로 설정되어 있습니다.
OCR/파싱 관련 버전은 `constraints.txt`로 고정됩니다.
기존 가상환경에서 업그레이드 시 누락을 막으려면 `pip install -r requirements-dev.txt --upgrade`를 권장합니다.

PDF OCR 속도 튜닝:

- 기본값은 `pdf_ocr_strategy=rapid` (빠른 처리 우선)
- 옵션: `off`, `rapid`, `easy`, `hybrid`
- `pdf_ocr_min_chars` 이하로 추출될 때만 OCR을 시도합니다.
- PDF 레이아웃 전략: `pdf_layout_model_strategy=off`
  - `off`: PDF에서 VLM 비활성화
  - `auto`: 그림/표가 감지되면 SmolDocling VLM 우선
  - `smoldocling`: PDF에서 항상 SmolDocling VLM 우선

이미지 처리 전략:

- 기본값은 `image_processing_strategy=rapid`
- 옵션: `rapid`, `smoldocling`, `hybrid`
- `smoldocling` 사용 시 VLM(예: SmolDocling 256M) 경로를 사용합니다.
- 기본 `image_vlm_device`는 `auto`입니다.
- `hybrid`는 RapidOCR를 먼저 시도하고, 실패 시 SmolDocling으로 fallback합니다.
- 실제 GPU 가속을 위해서는 CUDA 지원 PyTorch 런타임이 설치되어 있어야 합니다.

OCR warm-up endpoint:

- `POST /api/v1/files/warmup-ocr`
- 서버 시작 직후 한 번 호출하면 첫 업로드 지연(모델 로딩)을 줄일 수 있습니다.

RAG reset endpoint:

- `POST /api/v1/rag/reset`
- 제너레이션 초기화 시 업로드 문서/벡터 인덱스를 함께 비울 때 사용합니다.
...
- LangGraph:
  - `app/flows/copy_generation/graph.py` (호환 alias: `app/flows/copy_generation_graph.py`)
  - `app/flows/copy_lite_generation/graph.py` (호환 alias: `app/flows/copy_lite_generation_graph.py`)
  - `app/flows/file_upload_graph.py`
  - `app/flows/rag_workflow/graph.py` (호환 alias: `app/flows/rag_graph.py`)
  - `app/flows/web_workflow/graph.py` (호환 alias: `app/flows/web_graph.py`)
  - `app/flows/history_graph.py`
  - `app/flows/export_graph.py`
  - `app/flows/meta_graph.py`
  - 통합 테스트용 registry: `app/flows/graph_registry.py`
- LangChain:
  - `app/services/copy_service.py` (`_run_chain_or_fallback`)
  - `app/services/copy_service.py` (`_parse_prompt`, copy-lite parsing)
- API 진입: `app/api/v1/*.py`
...
## 4-1) Milvus 설정 (langchain-milvus)

RAG를 Milvus로 사용하려면 `MILVUS_URI`를 설정하세요.

- 예시 (`.env`):
  - `MILVUS_URI=http://127.0.0.1:19530`
  - `MILVUS_COLLECTION=copyjoe_docs`
- `MILVUS_URI`가 비어 있으면 메모리 백엔드로 동작합니다.
- 실제 연결 여부는 `/health`의 `rag_backend` 필드로 확인할 수 있습니다 (`milvus` 또는 `memory`).

호환성 오류를 줄이기 위해 Milvus/LangChain 조합 버전은 고정했습니다 (`requirements.txt`).
...
주의: 전역 파이썬 환경에 `langchain-community`, `langserve`, `langchain-tavily` 같은 0.3 계열 패키지가 이미 설치되어 있으면
`langchain 1.x`와 충돌할 수 있습니다. 반드시 프로젝트 전용 가상환경에서 `pip install -r requirements-dev.txt`로 설치하세요.
...
- `app/services/copy_service.py`는 파싱용 설정(`COPY_PARSER_*`)도 함께 읽어 바인딩합니다.
...
- `DOC_CONVERSION_FAILED` + `Converted text is empty ... OCR is required.`
  - 의미: 스캔/이미지형 PDF라 텍스트 레이어가 없어 OCR이 필요
  - 조치: OCR 처리된 PDF로 재저장 후 업로드하거나, 서버 환경에 docling OCR 의존성이 정상 설치되었는지 확인
...
- 그래프 통합 스모크 테스트: `notebooks/graph_registry_smoke_test.ipynb`
  - 실행: `python -m jupyter nbconvert --to notebook --execute --output graph_registry_smoke_test.executed.ipynb notebooks/graph_registry_smoke_test.ipynb`
