...
async def calculate_saju(
    ...
    Args:
        ...
        month: 출생월 (1-12)
        day: 출생일 (1-31)
        hour: 출생시간 (0-23, 기본값: 0)
        minute: 출생분 (0-59, 기본값: 0)
        is_male: 성별 (True: 남자, False: 여자, 기본값: True)
        is_leap_month: 윤달 출생 여부 (기본값: False)
...
@mcp.tool()
async def parse_saju_input(text: str) -> str:
    """
    자연어로 입력된 생년월일시 정보를 파싱합니다.
    
    Args:
        text: 자연어로 입력된 생년월일시 정보 (예: '1995년 8월 26일 오후 3시 30분 남자')
    
    Returns:
        str: 파싱된 결과 또는 오류 메시지
    """
    try:
        if not text:
