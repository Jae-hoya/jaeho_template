...
# 8글자 등장 횟수 방식, 각 오행의 합이 8이 안 되면 오류 발생
# 윤달 확인 링크: http://xn--vj1b09xs1b16ct2c.com/share/calendar/?selboxDirect=&y=1995&m=8&d=16
...
def get_wuxing_score_8chars(pillars: list) -> dict:
    """
    pillars: ['乙', '亥', '癸', '卯', '丙', '辰', '丙', '午'] 처럼 8글자(천간4+지지4)
    return: {'목': {'score': 2, 'chars': ['乙', '卯']}, ...}
    """
    # 오행 매핑표
    wuxing_map = {
        '목': ['甲', '乙', '寅', '卯'],
    ...
    # 점수 집계
    result = {k: {'score': 0, 'chars': []} for k in wuxing_map}
    for ch in pillars:
        element = char2wuxing.get(ch)
        if element:
            result[element]['score'] += 1
            result[element]['chars'].append(ch)
        else:
            raise ValueError(f"오행 매핑표에 없는 글자: {ch}")

    # 총합 검증
    total = sum(result[k]['score'] for k in result)
    if total != 8:
        raise ValueError(f"총합이 8이 아님: {total} (입력: {pillars})")
