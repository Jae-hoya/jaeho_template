...
def rag_search(
    ...
    rows = service.search(query=payload.query, top_k=payload.top_k, document_ids=payload.document_ids)
...
def rag_reset(service: RagService = Depends(rag_service)) -> RagResetResponse:
