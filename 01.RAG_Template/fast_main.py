...
def submit_conversation_individual_async(conversation_data):
    """thread_id별로 개별 대화를 FastAPI 서버에 전송"""
    def _send_individual():
        try:
            ...
            if response.status_code == 200:
                print(f"개별 대화 저장 성공: thread_id={conversation_data['thread_id']}, question_count={conversation_data['question_count']}")
            else:
                print(f"개별 대화 저장 실패: {response.status_code}")
                
        except Exception as e:
            print(f"개별 대화 저장 오류: {e}")
    
    # 비동기로 실행
    st.session_state["thread_pool"].submit(_send_individual)
    ...
    def _send_summary():
        try:
            ...
            conversation_summary = {
                ...
                "session_duration": 0,  # last_send_time이 주석 처리되어 있어서 0으로 설정
                "conversation_count": len(st.session_state["messages"]),
                "timestamp": datetime.now().isoformat()
            }
            
            # 개별 저장으로 변경되어 남은 버퍼가 없음
...
if clear_btn:
    ...
    st.session_state["open_feedback"] = False
    st.session_state["question_count"] = 0
    # st.session_state["conversation_buffer"] = []  # 개별 저장으로 변경되어 사용하지 않음
    # st.session_state["last_send_time"] = datetime.now()  # 개별 저장으로 변경되어 사용하지 않음
