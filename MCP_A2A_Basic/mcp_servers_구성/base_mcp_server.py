...
FASTMCP도 매우 편하지만,
여러개의 기능을 빨리빨리 만들어내야 한다면 **규칙적으로 코드를 만들어내기를 원하기 때문에**, 추상클래스를 정의해서 상속받아서 사용하는 방식을 사용한다.
그렇다면 이 안에만 채우게 되면 되는것이다. 이것은 FastMCP 규격에 맞춰서 코드를 작성하는 것이다.
"""
...
class BaseMCPServer(ABC):
    ...
    def __init__(
        ...
        server_name: str,
        debug: bool = False,
        transport: Literal["streamable-http", "stdio"] = "streamable-http", # 기본값을 streamable-http로 설정. docker로 올릴것이기 때문이다.
    ...
    def create_standard_response(
        ...
        **kwargs,
        ...
        return response_model.model_dump(exclude_none=True) # dict형태로 return
    ...
    def create_app(self) -> StarletteWithLifespan:
        ...
        # fastmcp에서 파이썬을 비동기로 쓰기위한 http_app(스탈렛) : StarletteWithLifespan. FastAPI도 스탈렛을 통해 어플리케이션화 한것이다.
