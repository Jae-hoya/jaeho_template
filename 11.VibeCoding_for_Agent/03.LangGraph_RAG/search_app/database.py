...
class Database:
    ...
    def execute(self, query: Any, params: Optional[tuple] = None):
        """Execute a query"""
        if not self.cur:
    ...
    def get_vector_db_info(self) -> Optional[Dict[str, int]]:
        """Get basic vector DB stats"""
        info_query = f"""
        ...
        try:
            ...
            if self.conn:
                self.conn.rollback()
            print(f"Warning: Could not fetch vector DB info: {e}")
            return None
