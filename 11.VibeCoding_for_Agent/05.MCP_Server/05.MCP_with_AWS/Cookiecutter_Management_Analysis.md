https://github.com/awslabs/mcp/tree/cookiecutters

Cookiecutter는 원래 Python 생태계에서 널리 사용되는 프로젝트 템플릿 도구인데, AWS에서 이를 활용해 MCP 서버 개발을 위한 boilerplate 코드를 자동 생성해주는 템플릿을 제공합니다.
AWS MCP Cookiecutter가 해주는 일:

MCP 서버의 기본 프로젝트 구조 생성
AWS 서비스 연동에 필요한 설정 파일들 자동 구성
인증, 에러 핸들링 등 공통 로직 포함
TypeScript 또는 Python 기반 선택 가능
