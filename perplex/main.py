...
with st.sidebar:
    ...
    st.markdown("made by Jae.hoya)")

    # 모델 선택 메뉴
    selected_model = st.selectbox("LLM 선택", ["gpt-4o", "gpt-4o-mini","gpt-4.1-mini"], index=2)
...
if apply_btn:
    tool = web_search_tool(
        ...
        include_domains=st.session_state["include_domains"] or None,
        # 필요 시 include_answer / include_raw_content / include_images 등도 여기서 설정
    )
    st.session_state["react_agent"] = create_agent_executor(
        model_name=selected_model,
        tools=[tool],
